...
export default function FluffyFlap({ onGameComplete }) {
  ...
  onSceneChangeRef.current = ({ season, night }) => {
    setSceneLabel({ key: `${season.id}-${night}-${Date.now()}`, text: `${season.emoji} ${season.name} · ${night ? "Night" : "Day"}` });
  };
  ...
  const setup = useCallback((k) => {
    ...
    function buildScene(canvases) {
      ...
      const player = addFluffy(k, {
        pos: k.vec2(PLAYER_X, STAGE_HEIGHT / 2),
        radius: PLAYER_R,
        comps: [
      ...
      const state = { speed: START_SPEED, distance: 0, elapsed: 0, lastGapY: STAGE_HEIGHT / 2, dead: false, colorIndex: 0, gates: 0 };

      function spawnGate() {
        const minY = GAP / 2 + 50;
        const maxY = STAGE_HEIGHT - GAP / 2 - 50;
        const rng = rngRef.current;
        const gapY = k.clamp(state.lastGapY + (rng() * 2 - 1) * MAX_GAP_SHIFT, minY, maxY);
        state.lastGapY = gapY;
        const color = k.rgb(GATE_COLORS[state.colorIndex++ % GATE_COLORS.length]);
        ...
        const group = gatePool.find((g) => g.pos.x < -GATE_WIDTH - 20) ?? addGateGroup();
        group.removeAll();
        group.pos = k.vec2(STAGE_WIDTH + 10, 0);

        // Top gate starts above the stage so only its bottom corners show rounded.
        group.add([k.pos(0, -30), k.rect(GATE_WIDTH, top + 30, { radius: 18 }), k.color(color), k.outline(3, k.rgb(255, 255, 255)), k.area(), "gate"]);
        group.add([k.pos(0, bottom), k.rect(GATE_WIDTH, STAGE_HEIGHT - bottom + 30, { radius: 18 }), k.color(color), k.outline(3, k.rgb(255, 255, 255)), k.area(), "gate"]);
        group.add([k.pos(GATE_WIDTH / 2, top), k.rect(4, GAP), k.area(), k.opacity(0), "scoreZone"]);
        ...
        if (scenery.currentSeason().id === "winter") {
          group.add([k.pos(-3, top - 11), k.rect(GATE_WIDTH + 6, 13, { radius: 6 }), k.color(255, 255, 255)]);
          group.add([k.pos(-3, bottom - 3), k.rect(GATE_WIDTH + 6, 13, { radius: 6 }), k.color(255, 255, 255)]);
          [12, 30, 48].forEach((dx) => group.add([k.pos(dx, top + 1), k.circle(4), k.color(255, 255, 255)]));
        }

        if (rng() < TREAT_CHANCE) {
          const treat = gameAssets.treats[Math.floor(rng() * gameAssets.treats.length)];
          const { width, height } = canvases[treat.id];
          const scale = TREAT_SIZE / Math.max(width, height);
          group.add([
            k.pos(GATE_WIDTH / 2, gapY + (rng() < 0.5 ? -1 : 1) * (28 + rng() * 20)),
            k.sprite(treat.id, { width: width * scale, height: height * scale }),
            k.anchor("center"),
            k.area(),
            k.z(1),
            "treat",
      ...
      k.onUpdate(() => {
        ...
        if (player.pos.y > STAGE_HEIGHT + 30) die();
        if (state.dead) return;

        state.elapsed += dt;
        state.speed = Math.min(MAX_SPEED, START_SPEED + state.elapsed * SPEED_RAMP);
      ...
      k.onUpdate("gateGroup", (group) => {
        ...
        if (group.pos.x >= -GATE_WIDTH - 20) group.pos.x -= state.speed * k.dt();
      });
      ...
      resetSceneRef.current = () => {
        ...
        Object.assign(state, { speed: START_SPEED, distance: GATE_SPACING - 60, elapsed: 0, lastGapY: STAGE_HEIGHT / 2, dead: false, gates: 0 });
        player.pos = k.vec2(PLAYER_X, STAGE_HEIGHT / 2);
      ...
      result={
        status === "result" && result ? (
          <GameResult
            ...
            shareText={result.mode === "daily" ? `Fluffy Flap · Daily ${result.date}: ${result.gates} gates, ${result.score} points ☁️ Same gates for everyone today!` : undefined}
        ...
        <div className="relative aspect-[19/28] w-full max-w-[380px] overflow-hidden rounded-2xl">
          ...
          {status === "idle" && (
            <div className="bg-fluffy-cream/90 absolute inset-0 flex flex-col items-center justify-center gap-4 p-6 text-center">
              <h2 className="font-poppins text-fluffy-text text-xl font-bold">Help Fluffy fly!</h2>
              <p className="text-fluffy-subtext max-w-xs text-sm">
                Tap anywhere to flap. Glide through the gaps and grab the treats for bonus points.
              </p>
